...
- 기본 사진 분석은 아래의 **실제 OpenAI Batch API**를 사용합니다. 사진은
  먼저 안전하게 저장되고 결과가 준비되면 앱에 반영됩니다. OpenAI의 완료 창은
  24시간이며, 일반 동기 처리 대비 입력·출력 토큰 가격이 50% 할인됩니다.
- 별도로 즉시 분석을 선택하면 앱과 서버가 최대 10개씩 실행합니다. 서버 동기
  대기열은 8개·30초로 제한되어 포화 상태의
  대형 이미지가 메모리에 계속 쌓이지 않습니다.
- 동시 실행 개수를 늘리면 처리량이 늘지만 할인은 생기지 않습니다. 할인은
  `/v1/batches`로 제출한 사진 분석에 적용되며, 별도의 장소 보강·계획 추천
  등 동기 호출은 기존 과금 방식입니다.
- 완전히 같은 이미지·프롬프트·모델·스키마·메타데이터·태그 어휘 요청은 진행
  중인 호출 하나를 함께 기다립니다. 검증에 성공한 결과만 프로세스 메모리에
  24시간, 최대 256개 보관하며 서버 재시작 시 사라집니다.
- 분석 지시문은 사진별 데이터보다 앞에 고정하고 Responses API의 프롬프트 캐시
  라우팅을 사용합니다. 서로 다른 사진도 공통 입력 부분의 캐시가 적용될 수
  있지만, 출력 토큰과 이미지별 고유 입력은 그대로 과금됩니다.
- 처리량·캐시 적중·토큰 수 집계는 기본 상태 응답에 노출하지 않습니다. 로컬
  진단이 필요할 때만 `TRUN_ON_DEBUG_METRICS=1 npm run dev`로 실행하면
  `/health`의 `analysis` 항목에서 확인할 수 있습니다. 이미지, 태그, URL, 캐시
  키는 포함하지 않습니다.

공식 계약: [Batch API](https://developers.openai.com/api/reference/resources/batches),
...
```json
{
  "requestId": "<64자리 소문자 SHA-256 형식의 안정적인 요청 ID>",
  "input": "<위 /v1/analyze와 같은 이미지·캡처·태그 객체>"
}
```

`input`은 문자열이 아니라 실제 객체입니다. 앱은 첫 제출 전에 요청 ID와
입력을 보존하고, 연결이 끊겨도 같은 ID로 재전송합니다. 성공 응답은 `202`와
`{requestId, status, createdAt, updatedAt}`입니다. 동일 ID의 동일 유효 입력은
기존 작업을 반환하고, 유효 이미지·모델·프롬프트·태그가 바뀌면 `409
REQUEST_ID_CONFLICT`를 반환합니다. 실패한 작업을 사용자가 새로 분석할 때는
새 ID를 사용합니다. 대기 중인 동일 유효 입력은 ID가 달라도 하나의 모델
작업으로 합칩니다.

### `POST /v1/analysis-tasks/status`

```json
{ "requestIds": ["<요청 ID, 1~100개, 중복 없음>"] }
```

응답은 `{tasks:[...]}`이며 입력 ID 순서를 유지합니다. 상태는 `queued`,
`submitted`, `in_progress`, `completed`, `failed`, `expired`, `cancelled`,
`submission_unknown`, `missing`입니다. 완료 작업에는 기존 `/v1/analyze`와
동일하게 검증한 `result`가 있고, 오류는 민감한 원문 없이
`error:{code,retryable}`로 전달합니다. 서버에 없는 ID는 `missing`입니다.

서버는 약 5초 동안 새 사진을 모아 최대 100장·180 MiB의 JSONL로 제출합니다.
각 행은 `{custom_id,method:"POST",url:"/v1/responses",body}`이며, 파일은
`purpose=batch`로 업로드하고 `/v1/batches`에 `completion_window:"24h"`로
생성합니다. 75장을 보내도 75개의 동기 모델 요청을 실행하지 않습니다.
큰 사진으로 파일 제한에 먼저 도달하면 여러 Batch로 나눕니다. 한 번에 하나의
그룹만 업로드/생성하며, 출력 파일은 한 줄씩 읽어 `custom_id`로 사진과 맞춥니다.
완료 순서는 보장되지 않으며, 만료·취소된 묶음에서도 완료된 사진은 저장합니다.
실패하거나 대기 중인 작업을 자동으로 일반 동기 호출로 전환하지 않습니다.

상태 확인은 서버에서 최소 30초 간격으로 합쳐 실행합니다. 휴대폰의 상태
확인이 겹쳐도 같은 조회를 공유합니다. 앱이 닫혀 있어도 서버와 OpenAI의
작업은 계속 진행되고, 서버를 재시작하면 저장된 작업을 이어서 확인합니다.
로컬 개발 서버가 꺼져 있으면 앱의 결과 수신은 다시 연결될 때까지 기다립니다.

### Batch 저장·복구

- 개인 데이터 저장 위치는 Git에서 제외된 `server/.local/batch-jobs`입니다.
  디렉터리는 `0700`, 파일은 `0600`으로 만들며 원자적 교체와 디스크 동기화를
  거친 뒤 접수를 응답합니다. HTTP 접수 대기 이미지는 디스크에 보관하고,
  파일 업로드 확인 후 개별 이미지와 로컬 JSONL을 지웁니다. 재시작 시 중단된
  삭제도 이어서 정리합니다. 이미지나 분석 결과를 로그에 남기지 않습니다.
- 한 저장소는 한 서버 프로세스만 사용합니다. 정상 종료는 잠금을 해제하며
  죽은 프로세스의 잠금은 복구 잠금 아래에서 다시 확인하고 교체합니다.
  잠금 소유자 파일이 없거나 복구 잠금 자체가 중단된 경우에는 중복 과금을
  막기 위해 시작을 거부합니다. 이 드문 경우는 운영자가 실행 중인 서버가
  없는지 확인한 후 해당 `.lock`/`.lock.recovering`만 정리해야 합니다.
- 대기 작업은 최대 1,000개, 로컬 이미지 대기 용량은 최대 2 GiB,
  접수 기록은 최대 10,000개입니다. 한도에 도달하면 `429 BATCH_QUEUE_FULL`로
  새 접수를 거절합니다. 한도를 올리기 전에 디스크 여유와 보관 정책을 확인하세요.
- 유효한 분석 결과는 완료 후 7일간 보관합니다. 이후 결과 내용은 지우고
  `expired / BATCH_RESULT_EXPIRED` 영수증만 남깁니다. 작은 접수 기록은 자동으로
  지우지 않아서 오래된 요청 ID를 재전송해도 다시 과금되지 않습니다. 기록 한도에
  도달하면 운영자가 앱 동기화 완료 여부를 확인하고 보관·삭제 정책을 결정합니다.
- 결과가 로컬에 안전하게 저장된 후 OpenAI 입력·출력·오류 파일을 삭제합니다.
  삭제 실패는 30분 간격으로 재시도합니다. 업로드 응답 자체가 유실되어 파일 ID를
  알 수 없는 경우는 OpenAI에 고아 파일이 남을 수 있어 별도 운영 정리가 필요합니다.
- 유료 Batch 생성 직전에 로컬 그룹 ID와 `creating`을 영구 저장합니다.
  생성 응답 유실·시간 초과·재시작 때는 OpenAI Batch 목록의 동일
  `metadata.local_group_id`와 입력 파일 ID로 기존 작업을 찾습니다.
  최근 최대 2,000개에서 정확히 하나를 확인하지 못하면 `submission_unknown`으로
  남겨 두고 생성 요청을 반복하지 않습니다. 자동 재과금 대신 운영 확인이 필요한
  상태이며, 새로운 요청 ID로 우회 재분석하기 전에 기존 Batch 존재를 확인해야 합니다.

이 서버는 단일 개발 사용자를 위한 로컬 저장소입니다. 공개 서비스 배포에는
사용자 인증, 계정별 작업 소유권과 디스크 저장소, 삭제 정책을 적용해야 합니다.
