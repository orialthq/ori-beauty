...
- 앱은 사진 분석을 최대 3개씩 실행하고, 서버는 모든 클라이언트를 합쳐 OpenAI
  요청을 최대 4개만 실행합니다. 서버 대기열은 8개·30초로 제한되어 포화 상태의
...
- 처리량·캐시 적중·토큰 수 집계는 기본 상태 응답에 노출하지 않습니다. 로컬
  ...
  키는 포함하지 않습니다.
- OpenAI Batch API는 24시간 완료 창을 사용하는 비동기 작업이므로, 사진을 넣고
  바로 결과를 보는 이 경로에는 사용하지 않습니다. 야간 재분석 같은 별도
  오프라인 작업이 생기면 그때 분리해 사용합니다.
